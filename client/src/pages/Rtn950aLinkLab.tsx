  ...
  return <main className="rtnlab-page" dir="ltr">
    ...
    <section className="rtnlab-shell">
      ...
      <section className="rtnlab-main">
        <div className="rtnlab-tabs"><button>Slot Layout</button><button className="active">Microwave Link Configuration <X size={12}/></button><button>Physical Link Aggregation</button></div>
        ...
        <section className="rtnlab-workspace" data-testid="rtnlab-workspace" data-active-step={step}>
          ...
          {step === "confirm" && <ConfirmApply endpoints={endpoints} onRequest={() => setRiskOpen(true)}/>} 
...
function ConfirmApply({ endpoints, onRequest }: { endpoints: { siteA: string; siteB: string }; onRequest: () => void }) { return <Panel eyebrow="STEP 5 / RISK CONFIRMATION" title="ملخص المقارنة وطلب تنفيذ تدريبـي" actions={<button className="rtnlab-danger" onClick={onRequest}><AlertTriangle size={15}/> Apply training draft</button>}><div className="rtnlab-confirm-grid"><div><b>Site A / Near End</b><p>{endpoints.siteA}</p><p>Training draft endpoint</p></div><div><b>Site B / Far End</b><p>{endpoints.siteB}</p><p>Training comparison endpoint</p></div><div><b>تغيير مشترك</b><p>Link 1 / RTN950A 2+0 Training Profile</p><p>Basic Parameters + RF Information</p></div><div><b>الحدود</b><p>لا توجد جلسة NE، ولا اتصال ODU، ولا كتابة إعداد، ولا مقارنة قياسات حية.</p></div><div><b>المصدر المرئي</b><p>يعرض التحذير عند Apply لأن تغيير المعاملات قد يكسر الوصلة. هنا يُعاد تمثيله كتحذير تدريبي فقط.</p></div></div></Panel>; }
